...
describe('Wizard', function() {
  ...
  it('should start rested', function() {
    // create wizard
    var wizard = new Wizard({name: 'Foster', bearded: false});
  ...
  it('should be able to cast spells', function() {
    // create wizard
    var wizard = new Wizard({name: 'Kate', bearded: false});
  ...
  it('should only be able to cast 3 spells', function() {
    // create 
    var wizard = new Wizard({name: 'Jordan'});
    // assert isRested is true
    assert.equal(wizard.isRested, true)
    // cast()
    wizard.cast('magic bullet');
    // assert isRested is true
    assert.equal(wizard.isRested, true)
    // cast()
    wizard.cast('peanut butter');
    // assert isRested is true
    assert.equal(wizard.isRested, true)
    // cast()
    wizard.cast('wand wave');
