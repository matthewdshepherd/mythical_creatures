...
describe('Medusa', function() {
  ...
  it.skip('should unstone the the first victim and stone the 4th coming in', function() {
    ...
    // create first victim
    var victim1 = new Person('Quinne');
    // create second victim
    var victim2 = new Person('Kate')
    // create third victim
    var victim3 = new Person('Foster')
    // create fourth victim
    var victim4 = new Person('Jordan')

    // medusa stares at first victim
    medusa.stare(victim1)
    // assert that the first victim is stoned
    assert.equal(medusa.statues[0].name, 'Quinne')
    ...
    assert.equal(medusa.statues[2].name, 'Jordan')
    // assert that the first victim is not stoned
    assert.equal(medusa.statues[0].name, 'Kate')
  });
})
